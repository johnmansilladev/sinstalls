$("#tableVersionSoftwares tbody tr").dblclick(function () {
    ...
    axios
        ...
        .then(function (response) {
            ...
            let data = response.data.software.version_softwares;
            let infoSoftware =  response.data.software;
            let appendInfo  = `
                <div class="row">
                    <h4 class="fw-bolder">${infoSoftware.soft_nombre}</h4>
                </div>`;
            let tbody = "";
            data.forEach((obj) => {
                tbody += `<tr>
...
function emptyInfoSoft() {
    $("#infoSoftware div").remove();
}
function emptyInfo(){
    emptyTBody();
    emptyInfoSoft();
